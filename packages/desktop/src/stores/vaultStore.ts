...
export interface VaultActions {
  ...
  addEntry: (
    entry: Omit<Entry, 'id' | 'createdAt' | 'updatedAt' | 'deletedAt'>,
  ) => Promise<Entry>;
  ...
  addCategory: (
    category: Omit<Category, 'id' | 'updatedAt' | 'deletedAt'>,
  ) => Promise<Category>;
...
export const useVaultStore = create<VaultStore>((set, get) => ({
  ...
  addEntry: async (
    entryData: Omit<Entry, 'id' | 'createdAt' | 'updatedAt' | 'deletedAt'>,
  ) => {
    const {entries} = get();
    const id = generateId();
    const now = new Date();
  ...
  deleteEntry: async (id: string) => {
    ...
    const updatedEntries = entries.map((entry: Entry) =>
      entry.id === id
        ? {...entry, deletedAt: new Date(), updatedAt: new Date()}
        : entry,
    );
    set({entries: updatedEntries});
  ...
  addCategory: async (
    categoryData: Omit<Category, 'id' | 'updatedAt' | 'deletedAt'>,
  ) => {
  ...
  deleteCategory: async (id: string) => {
    ...
    const updatedCategories = categories.map((cat: Category) =>
      cat.id === id
        ? {...cat, deletedAt: new Date(), updatedAt: new Date()}
        : cat,
    );
  ...
  importData: async (
    ...
    const mergedCategories = Array.from(
      mergeResultCategories.categories.values(),
    );
